...
// Loads static GeoJSON generated by the pipeline — no Edge Function needed.
...
async function loadTripsGeoJSON() {
  const url = CONFIG.TRIPS_GEOJSON_URL;
  ...
  try {
    ...
    // Client-side filter: only show trips from the last INITIAL_DAYS days
    if (CONFIG.INITIAL_DAYS && geojson.features?.length) {
      const since = new Date();
      since.setDate(since.getDate() - CONFIG.INITIAL_DAYS);
      const sinceMs = since.getTime();

      geojson.features = geojson.features.filter(f => {
        const ts = f.properties?.timestamp || f.properties?.['GNSS Timestamp'];
        if (!ts) return true; // keep features with no timestamp
        return new Date(ts).getTime() >= sinceMs;
      });
    }

    console.log(`✅ Loaded ${geojson.features?.length ?? 0} segments`);
    return geojson;
  } finally {
